...
4. Grid convergence study shows observed order of accuracy ≥ 1.8 (target 2.0) under uniform mesh refinement on VAL-001.
   ...
   *Note 2026-09-25, criterion 4: the order judged.* The criterion names no reference, and the order is judged against none: from three uniform grids, 40x20, 80x40 and 160x80, as p = log2(|f40 - f80| / |f80 - f160|) on the u profile at x = L/2 exactly, fine profiles restricted to the coarse rows by averaging pairs (`scripts/val001_order.py`). The reason is that the analytical parabola is not the exact answer at L/2. The flow is still developing there, and the difference between L/2 and 3L/4, about 1.1e-4 of the parabola's norm, does not shrink under refinement (`docs/reports/stopping_rule_evidence.md`, section 3). Against the parabola the order would measure the approach to that floor, not the scheme. The orders against the parabola at L/2 and at 3L/4 are reported beside the judged one. Decided by the orchestrator on 2026-09-25; no threshold changes.
